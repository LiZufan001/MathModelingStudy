...
问题一首先检查完整拓扑序、buffer 的 ALLOC--USE--FREE 生命周期以及 L0A/L0B/L0C 的同类单驻留约束，再由独立 evaluator 仅根据输出 schedule 重新累计 $R_k$ 与 $R_{\max}$。小规模随机图使用 exact solver/oracle 对照，用于发现 ready-set 处理、生命周期边界和 L0 prerequisite 等实现错误；六组 Appendix-E 大实例则统一运行四个确定性策略并重新比较正式峰值。
