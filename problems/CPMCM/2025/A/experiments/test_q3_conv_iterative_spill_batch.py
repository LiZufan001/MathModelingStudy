...
def test_iterative_batch_fresh_reranks_until_saturation() -> None:
    graph, solution = _fixture()
    result = optimize_q3_critical_spill_batch_iterative(
        graph,
        solution,
        max_rounds=4,
        max_switches=4,
...
def test_iterative_batch_rejects_empty_prefix_portfolio() -> None:
    graph, solution = _fixture()
    try:
        optimize_q3_critical_spill_batch_iterative(
            graph,
            solution,
            max_switches=4,
            prefix_sizes=(8, 16),
