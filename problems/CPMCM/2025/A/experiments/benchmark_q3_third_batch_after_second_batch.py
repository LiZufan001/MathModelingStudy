...
def main() -> int:
    ...
    # Reconstruct the exact second-batch winner from the current post-polish
    # state.  This avoids trusting only the JSON evidence: the candidate is
    # rebuilt and revalidated under the current implementation before the next
    # search starts.
    ...
    final_q2 = validate_q2_solution(graph, third_batch.best_solution)
    final_q2.require_ok()
    base_spills = tuple((s.buf_id, s.new_offset) for s in current.spills)
    final_spills = tuple((s.buf_id, s.new_offset) for s in third_batch.best_solution.spills)
    if final_spills != base_spills:
        raise AssertionError("third batch changed spill records")
    if final_q2.spill_count != source_q2.spill_count or final_q2.extra_traffic != source_q2.extra_traffic:
        raise AssertionError("third batch changed Q2 traffic/spill count")
    ...
    payload = {
        ...
        "second_batch_replayed": True,
        "second_batch_official_cycles": second_batch.best_official.total_cycles,
        "second_batch_safe_cycles": second_batch.best_safe.total_cycles,
        "third_batch_improved": third_batch.improved,
        "third_batch_best_prefix_size": third_batch.best_prefix_size,
        "final_official_cycles": third_batch.best_official.total_cycles,
        "final_safe_cycles": third_batch.best_safe.total_cycles,
        "third_batch_improvement_cycles": (
            second_batch.best_official.total_cycles - third_batch.best_official.total_cycles
        ),
        "improvement_vs_saturated_cycles": SATURATED_REFERENCE - third_batch.best_official.total_cycles,
        "spill_count": final_q2.spill_count,
        "extra_traffic": final_q2.extra_traffic,
        "safe_overlap_errors": len(third_batch.best_safe.physical_overlap_errors),
        "valid": third_batch.best_official.ok and third_batch.best_safe.ok,
        "source_validation_seconds": round(t1 - t0, 6),
        "polish_exact_replay_seconds": round(t2 - t1, 6),
        "second_batch_exact_replay_seconds": round(t3 - t2, 6),
        "third_batch_seconds": round(t4 - t3, 6),
        ...
        "trials": [
            ...
            for trial in third_batch.trials
        ],
    }
