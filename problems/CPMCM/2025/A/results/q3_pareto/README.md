...
`promoted Q2 -> formal zero-traffic core -> fresh-rerank critical-SPILL batch`

整个第二阶段保持 promoted Q2 的 SPILL 身份/顺序、spill count、extra traffic 不变，只调整 Q3 调度序列；每个候选都重新执行 strict Q2、official-literal 与 residency-safe replay。
...
| Conv_Case0 | 177,904 | 603,405 | **597,969** | **-5.996567%** | 787,783 |
| Conv_Case1 | 721,464 | 3,781,664 | **3,767,326** | **-2.211968%** | 4,112,665 |

critical-SPILL batch 对 Matmul0、Matmul1、FA1 第一轮即 no-op；对 FA0、Conv0、Conv1 分别额外降低 `617 / 5,436 / 14,338` cycles。六组最后一轮均为 no-improvement，因此对当前算子具有局部饱和证据。
...
FA1 的 formal fixed-traffic 点更新为 `242,552 / 962,022`，而两个更高 traffic / 更低 cycles 的 composition 点仍然不被支配：
...
- 五组非 Conv1 正式附件导出：run `34747455829`，5/5 success；
- Conv1 full formal：run `34745747473`，success；
- Conv1 artifact：`10314199173`；
- Conv1 digest：`sha256:dda2514dcb6764ddcfa8361ed513acd7494c7ab6c4fc1b0add09aecb95c2a7e7`。
...
- Conv1 的 final point 已由两条路线独立复现；
- refined frontier 已经过 strict-valid-only Pareto reconciliation。

但**不能**声称整个 Q3 全局最优。此前 single-switch greedy 长 run 被 timeout/cancel，没有形成 no-op 证明；新的 schedule operator、priority portfolio 或联合变换仍可能继续改善。

若继续 Q3，优先探索新的 operator，而不是继续增加当前 batch 深度。
